...
(function game(){
  ...
  const loop = () => {
    ...
    hearts.forEach(h => { h.y += h.vy; drawHeart(h.x,h.y,h.r); });
...
(async function autoGallery(){
  ...
  // Якщо джерела задані вручну — просто рендеримо їх без перевірок (file:// та деякі хости блокують fetch/HEAD)
  if (Array.isArray(sources) && sources.length) {
    ...
    return; // важливо: не йдемо у режим автопошуку
  }
...
(function fun(){
  ...
  btn?.addEventListener('click', () => {
    burstConfetti(900);
    btn.disabled = true;
    btn.textContent = 'ну я жжж казала, зайка)';
